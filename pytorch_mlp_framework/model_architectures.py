...
class ConvolutionalProcessingBlockBatchNorm(nn.Module):
    ...
    def build_module(self):
        ...
        self.layer_dict["batchnorm_0"] = nn.BatchNorm2d(num_features=out.shape[1])
        out = self.layer_dict['conv_0'].forward(out)
        out = self.layer_dict['batchnorm_0'].forward(out)
        out = F.leaky_relu(out)
        ...
        self.layer_dict["batchnorm_1"] = nn.BatchNorm2d(num_features=out.shape[1])
        out = self.layer_dict['conv_1'].forward(out)
...
class ConvolutionalProcessingBlockBatchNormResidual(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvolutionalProcessingBlockBatchNormResidual, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict["batchnorm_0"] = nn.BatchNorm2d(num_features=out.shape[1])#features in the number of channels
        ...
        self.layer_dict["batchnorm_1"] = nn.BatchNorm2d(num_features=out.shape[1])#features in the number of channels
...
class ConvolutionalDimensionalityReductionBlockBatchNorm(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ConvolutionalDimensionalityReductionBlockBatchNorm, self).__init__()
